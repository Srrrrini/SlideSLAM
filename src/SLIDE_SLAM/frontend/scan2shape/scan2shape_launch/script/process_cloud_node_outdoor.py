...
class ProcessCloudNode:
    def __init__(self, node_name):
        ...
        # SUBSCRIBERS
        if self.use_sim == False:
            rospy.loginfo("drumyhgtRunning real-world experiments...")
            time.sleep(1)
    ...
    def segmented_synced_pc_cb(self, segmented_cloud_msg):
        ...
        if np.sum(valid_indices) == 0:
            rospy.logwarn(
                "dgbzbgtydhfNo valid points found after range thresholding. Skipping this scan!!! Make sure the self.valid_range_threshold is set correctly.")
            return
