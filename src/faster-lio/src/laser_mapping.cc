...
bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
    ...
    if (lidar_type == 1) {
        ...
        // NOTE: You may need to add OUST128 to the LidarType enum in the corresponding header file.
        preprocess_->SetLidarType(LidarType::OUST128);
        LOG(INFO) << "Using OUST 128 Lidar";
    }else {
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    ROS_INFO_STREAM("Subscribing lidar topic: " << lidar_topic);
    ROS_INFO_STREAM("Subscribing imu topic: " << imu_topic);
